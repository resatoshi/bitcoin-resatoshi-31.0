...
const QString BITCOIN_IPC_PREFIX("resatoshi:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // resatoshi: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click resatoshi: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start resatoshi: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling resatoshi: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("resatoshi://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'resatoshi://' is not a valid URI. Use 'resatoshi:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // resatoshi: URI
    {
