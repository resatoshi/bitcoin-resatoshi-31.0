...
public:
    CMainParams() {
        ...
        // First four bytes of SHA256(genesis_timestamp), distinct from every
        // Bitcoin network message start.
        ...
        const char* genesis_timestamp = "ReSatoshi mainnet 04/Sep/2026 - A new chain from block zero";
        const CScript genesis_output_script = CScript()
            << "0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798"_hex
            << OP_CHECKSIG;
        // A one-logical-CPU grind benchmark measured 12,623,487 hashes in
        // 6.40 CPU seconds (1,972,419.84 H/s). Compact target 0x1d03a112
        // represents 1,183,453,623 expected hashes, or 600.00087 seconds.
        genesis = CreateGenesisBlock(genesis_timestamp, genesis_output_script,
                                     1788480000, 1930579877, 0x1d03a112, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256{"0000000379813245fcdd4600969c6f74684f14433badf837be5b8ffa560174ec"});
        assert(genesis.hashMerkleRoot == uint256{"d9effac57f561ba305e519c420626e3841d3b703cf3cfaa8ed1edb8a2b5b156e"});
        ...
        // Keep the standard WIF encoding for private-key and tooling compatibility.
        // ReSatoshi payment addresses remain separated by their distinct prefixes.
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1, 128);
        // Keep BIP32's standard xpub/xprv serialization for tool compatibility;
        // derived payment addresses still use ReSatoshi-specific prefixes.
...
public:
    explicit CRegTestParams(const RegTestOptions& opts)
    {
        ...
        consensus.recycle_enabled = opts.recycle;
        if (opts.recycle) consensus.recycle_expiry_blocks = 200;
